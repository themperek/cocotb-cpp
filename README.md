...
```bash
g++ -shared -std=c++20 -fPIC -O3 -I./src -I$(cocotb-config --share)/include tests/test_dff.cpp src/cocotb_core.cpp -o lib_dff.so
```
...
- **cocotb-cpp:** Testbench, driver, and transaction logic implemented fully in C++ with cocotb-cpp API (`tests/test_axil.cpp` + `examples/axil_ext/src/axil.h`).
- **cocotb-cpp + Python :** Python test via cocotb-cpp bridge (`tests/test_axil.py`)
- **cocotb-cpp + axil-cpp + Python :** Mixed testbench, Python test via cocotb-cpp bridge with AXI transtions in C++ (`examples/axil_ext/tests/test_axil_ext.py` + `examples/axil_ext/src/axil.h`).
